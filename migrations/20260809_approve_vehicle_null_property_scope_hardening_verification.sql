-- ══════════════════════════════════════════════════════════════════════
-- 20260809_approve_vehicle_null_property_scope_hardening_verification.sql
-- POST-APPLY: RPC still exists at same identity signature, p_manager_note
-- DEFAULT NULL preserved, grants preserved, both hardening layers
-- present in the function body, comment updated, schema audit row landed.
--
-- 🔴 RETURNS-ROWS PATTERN v2 (Mateo Aug 13 correction)
-- The `RAISE EXCEPTION`-only pattern (v0) used by prior verification
-- migrations produced silent success — Jose could not distinguish
-- "no rows because passed" from "no rows because I forgot to run".
--
-- v1 (Aug 9) added a final `SELECT 'PASS' ... ;` before `COMMIT;` —
-- but the Supabase SQL Editor returns the result of the LAST
-- statement in the paste, which was `COMMIT` (returns nothing). The
-- PASS row was generated and discarded before Jose could see it.
--
-- v2: **no BEGIN/COMMIT wrap.** This file is a read-only assertion
-- — every gate is a `DO $$ … END $$;` that RAISEs on failure and a
-- terminal `SELECT`. There is nothing to roll back on pass, and any
-- RAISE aborts the entire multi-statement paste in the SQL Editor
-- with the exception message visible. Jose sees exactly two
-- outcomes: PASS row on success, exception message on failure.
--
-- 🔴 Function lookup uses `pg_get_function_identity_arguments`
-- (stable identity signature, no DEFAULTs) so a future default tweak
-- on p_manager_note doesn't break the lookup. The default-preservation
-- check is its own VQ.DEFAULTS_PRESERVED gate below.
-- ══════════════════════════════════════════════════════════════════════
--
-- Run AFTER 20260809_approve_vehicle_null_property_scope_hardening.sql.
-- Paste WHOLE. Expect: one row `PASS | approve_vehicle(BIGINT,TEXT) | {8 gates} | <ts>`.
-- ══════════════════════════════════════════════════════════════════════


-- ── VQ.RPC_EXISTS ────────────────────────────────────────────────────
DO $$
DECLARE v_count INT;
BEGIN
  SELECT COUNT(*) INTO v_count
    FROM pg_proc p
    JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public'
     AND p.proname = 'approve_vehicle'
     AND pg_get_function_identity_arguments(p.oid) = 'p_vehicle_id bigint, p_manager_note text';
  IF v_count <> 1 THEN
    RAISE EXCEPTION 'VQ.RPC_EXISTS: expected 1 approve_vehicle(BIGINT,TEXT); got %', v_count;
  END IF;
END $$;

-- ── VQ.DEFAULTS_PRESERVED ────────────────────────────────────────────
-- 🔴 The 20260628 install carried `p_manager_note TEXT DEFAULT NULL::text`.
-- A signature drop would silently break one-arg callers with 42883
-- at call time (no earlier signal). Verify the default survived.
DO $$
DECLARE v_args_with_defaults TEXT;
BEGIN
  SELECT pg_get_function_arguments(p.oid) INTO v_args_with_defaults
    FROM pg_proc p
    JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public'
     AND p.proname = 'approve_vehicle'
     AND pg_get_function_identity_arguments(p.oid) = 'p_vehicle_id bigint, p_manager_note text';
  IF v_args_with_defaults NOT LIKE '%DEFAULT NULL%' THEN
    RAISE EXCEPTION 'VQ.DEFAULTS_PRESERVED: p_manager_note DEFAULT NULL was dropped by the re-definition; one-arg callers would break. Got: %', v_args_with_defaults;
  END IF;
END $$;

-- ── VQ.RPC_STILL_DEFINER ─────────────────────────────────────────────
DO $$
DECLARE v_security TEXT;
BEGIN
  SELECT CASE prosecdef WHEN true THEN 'DEFINER' ELSE 'INVOKER' END
    INTO v_security
    FROM pg_proc p
    JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public'
     AND p.proname = 'approve_vehicle'
     AND pg_get_function_identity_arguments(p.oid) = 'p_vehicle_id bigint, p_manager_note text';
  IF v_security <> 'DEFINER' THEN
    RAISE EXCEPTION 'VQ.RPC_STILL_DEFINER: expected SECURITY DEFINER; got %', v_security;
  END IF;
END $$;

-- ── VQ.LAYER_1_PROPERTY_PRESENCE_GATE ───────────────────────────────
DO $$
DECLARE v_body TEXT;
BEGIN
  SELECT pg_get_functiondef(p.oid) INTO v_body
    FROM pg_proc p
    JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public'
     AND p.proname = 'approve_vehicle'
     AND pg_get_function_identity_arguments(p.oid) = 'p_vehicle_id bigint, p_manager_note text';
  IF v_body NOT LIKE '%vehicle_property_missing%' THEN
    RAISE EXCEPTION 'VQ.LAYER_1_PROPERTY_PRESENCE_GATE: function body missing vehicle_property_missing error class; hardening did not land';
  END IF;
END $$;

-- ── VQ.LAYER_2_COALESCE_SCOPE_FLAG ──────────────────────────────────
DO $$
DECLARE v_body TEXT;
BEGIN
  SELECT pg_get_functiondef(p.oid) INTO v_body
    FROM pg_proc p
    JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public'
     AND p.proname = 'approve_vehicle'
     AND pg_get_function_identity_arguments(p.oid) = 'p_vehicle_id bigint, p_manager_note text';
  IF v_body !~* 'COALESCE\s*\(\s*v_in_scope\s*,\s*false\s*\)' THEN
    RAISE EXCEPTION 'VQ.LAYER_2_COALESCE_SCOPE_FLAG: function body missing COALESCE(v_in_scope, false); hardening did not land';
  END IF;
END $$;

-- ── VQ.GRANTS_PRESERVED ─────────────────────────────────────────────
DO $$
DECLARE v_anon_has BOOLEAN; v_auth_has BOOLEAN; v_public_has BOOLEAN;
BEGIN
  SELECT
    has_function_privilege('anon',          'public.approve_vehicle(bigint,text)', 'EXECUTE'),
    has_function_privilege('authenticated', 'public.approve_vehicle(bigint,text)', 'EXECUTE'),
    has_function_privilege('public',        'public.approve_vehicle(bigint,text)', 'EXECUTE')
  INTO v_anon_has, v_auth_has, v_public_has;
  IF v_anon_has THEN
    RAISE EXCEPTION 'VQ.GRANTS_PRESERVED: anon HAS EXECUTE on approve_vehicle (must be REVOKED)';
  END IF;
  IF v_public_has THEN
    RAISE EXCEPTION 'VQ.GRANTS_PRESERVED: PUBLIC HAS EXECUTE on approve_vehicle (must be REVOKED)';
  END IF;
  IF NOT v_auth_has THEN
    RAISE EXCEPTION 'VQ.GRANTS_PRESERVED: authenticated MISSING EXECUTE on approve_vehicle';
  END IF;
END $$;

-- ── VQ.COMMENT_UPDATED ──────────────────────────────────────────────
DO $$
DECLARE v_comment TEXT;
BEGIN
  SELECT obj_description(p.oid, 'pg_proc') INTO v_comment
    FROM pg_proc p
    JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public'
     AND p.proname = 'approve_vehicle'
     AND pg_get_function_identity_arguments(p.oid) = 'p_vehicle_id bigint, p_manager_note text';
  IF v_comment IS NULL OR v_comment NOT LIKE '%Item 2 hardening%' THEN
    RAISE EXCEPTION 'VQ.COMMENT_UPDATED: function comment missing Item 2 hardening reference';
  END IF;
END $$;

-- ── VQ.SCHEMA_AUDIT_ROW ─────────────────────────────────────────────
DO $$
DECLARE v_count INT;
BEGIN
  SELECT COUNT(*) INTO v_count
    FROM public.audit_logs
   WHERE action = 'SCHEMA_APPROVE_VEHICLE_NULL_PROPERTY_HARDENING'
     AND new_values->>'migration' = '20260809_approve_vehicle_null_property_scope_hardening';
  IF v_count < 1 THEN
    RAISE EXCEPTION 'VQ.SCHEMA_AUDIT_ROW: SCHEMA_ audit row missing';
  END IF;
END $$;

-- ── FINAL: return one row on pass ─────────────────────────────────
-- All eight DO blocks passed (any RAISE would have aborted the tx
-- before this SELECT). This row is the visible evidence of pass so
-- Jose can distinguish silent success from not-run.
SELECT
  'PASS'::TEXT                          AS status,
  'approve_vehicle(BIGINT,TEXT)'::TEXT  AS function_name,
  ARRAY[
    'RPC_EXISTS',
    'DEFAULTS_PRESERVED',
    'RPC_STILL_DEFINER',
    'LAYER_1_PROPERTY_PRESENCE_GATE',
    'LAYER_2_COALESCE_SCOPE_FLAG',
    'GRANTS_PRESERVED',
    'COMMENT_UPDATED',
    'SCHEMA_AUDIT_ROW'
  ]                                     AS gates_verified,
  now()                                 AS verified_at;
