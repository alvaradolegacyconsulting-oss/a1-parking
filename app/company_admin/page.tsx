...
export default function CompanyAdminPortal() {
  ...
  // heuristic at :3635 which returns green for "Failed..." with capital
  // F — see docs/backlog/ca-msgbox-severity-derived-from-text.md).
  ...
  // 2026-08-20 house-rules arc Commit 4 — CA read-only view + view-history.
  // Modal opens from the property detail card. Current text renders
  // via the shared HouseRulesRenderer (same pre-wrap treatment as
  // resident view). History fetches on demand (not on every property
  // detail render — the RLS-scoped query is cheap but not free).
  //
  // 🔴 Version + effective date VISIBLE on CA side (dispatcher use
  // case per Jose Aug 20: "version 3, effective 15 August" is the
  // detail that matters when the tow happened on the 12th).
  // Resident view deliberately hides version; CA needs it.
  const [caHouseRulesModalProperty, setCaHouseRulesModalProperty] = useState<any>(null)
  const [caHouseRulesHistory, setCaHouseRulesHistory] = useState<Array<{
    version: number
    text: string | null
  ...
  // 2026-08-20 house-rules arc Commit 4 — open the CA read-only
  // modal + fetch history on demand. History is scoped to this
  // property; RLS on property_house_rules_versions restricts to
  // the CA's own company (migration 20260820_property_house_rules_v1
  // §PART 3). Rows come back newest-first.
  //
  // Opens the modal immediately with current text (already on the
  // property row from the CRM properties fetch); fires the history
  // fetch async. History displays with a loading indicator until it
  // lands, so the CA gets the current text without waiting.
  async function caOpenHouseRulesModal(property: any) {
    setCaHouseRulesModalProperty(property)
    setCaHouseRulesHistory(null)   // clear previous
    setCaHouseRulesHistoryLoading(true)
    try {
      const { data } = await supabase
        .from('property_house_rules_versions')
        .select('version, text, effective_date, created_at, created_by_email')
        .eq('property_id', property.id)
        .order('version', { ascending: false })
      setCaHouseRulesHistory((data ?? []) as any)
    } finally {
      setCaHouseRulesHistoryLoading(false)
    }
  }

  function caCloseHouseRulesModal() {
    setCaHouseRulesModalProperty(null)
    setCaHouseRulesHistory(null)
  }
      ...
      if (role?.role !== 'admin') {
        ...
        if (!isUnderLimit(FEATURE_FLAGS.MAX_DRIVERS, currentActiveCount, ctx)) {
          ...
          const opened = offerForcedUpgrade(
            ...
            () => { setDriverActionResult(null); toggleDriverActive(driver) },
          )
          if (!opened) {
            setDriverActionResult({
              severity: 'error',
              text: `Driver limit reached (${limit}). Contact support to expand your account.`,
            })
          }
          return
        }
  ...
  async function saveCompanyLogo(newLogoUrl: string) {
    ...
    //   1. The compact-bar thumbnail's useResolvedLogo(...) at :158
    //      snapshots localStorage at render time. Without this cache
    //      update, setCompanyLogoUrl triggers a re-render but the
    //      hook sees the SAME localStorage snapshot → useEffect
    //      dep unchanged → thumbnail stays stale until next login.
    //   2. driver/page.tsx:1036 reads localStorage.company_logo at
    //      tow-ticket generate time. Without this update, A1's tow
    //      ticket renders the OLD logo until the CA logs out + back
    //      in (bootstrap re-fetches from companies.logo_url).
    // Both gaps close with the same one-line write.
    if (typeof window !== 'undefined') {
      try { localStorage.setItem('company_logo', newLogoUrl) } catch { /* quota / private mode — non-fatal */ }
    }
  }
  ...
  function printAllPropertyQRSigns() {
    ...
    //
    // 2026-07-15 QR cluster — parity with printQRSign single-sign path
    // (Bug 3 fallback URL added). Bulk-print is visitor-only today; if
    // resident bulk-print gets added, parameterize the same way as
    // printQRSign (kind='resident' swaps header/subhead/note/no-warn).
    const cards: string[] = []
    for (const prop of properties || []) {
      ...
      // Reconstruct the URL so the fallback text matches what's encoded
      // in the QR. Mirrors the single-sign path (visitor QR URL shape).
            ...
            {properties.map((prop, i) => {
              ...
              return (
                <div key={i} style={{ background:'#161b26', border:'1px solid #2a2f3d', borderRadius:'10px', padding:'20px', marginBottom:'12px', textAlign:'center' }}>
                  ...
                  {/* 2026-07-15 QR cluster — was an inline tw.document.write
          ...
          <div>
                    ...
                    <div style={{ background:'#161b26', border:'1px solid #2a2f3d', borderRadius:'12px', padding:'18px' }}>
                      {!selected ? (
                        ...
                        <>
                          ...
                          {/* 2026-08-20 house-rules arc Commit 4 — CA
                              read-only surface. Chip + View button.
                              Published-when-set, "Not published" pill
                              when null. Version + effective date on
                              the pill because CA is the dispatcher's
                              view (Jose Aug 20). Read-only throughout;
                              CA does not author. Absence of the View
                              button when unpublished is deliberate —
                              nothing to view. */}
                          <div style={{ marginBottom:'14px' }}>
                            <p style={{ color:'#C9A227', fontSize:'10.5px', textTransform:'uppercase', letterSpacing:'0.08em', margin:'0 0 6px', fontWeight:'bold' }}>House rules</p>
                            {selected.house_rules_text ? (
                              <div style={{ fontSize:'12.5px', color:'#aaa', display:'flex', alignItems:'center', gap:'10px', flexWrap:'wrap' }}>
                                <span style={{ background:'#1a3a1a', color:'#4caf50', padding:'2px 8px', borderRadius:'10px', fontSize:'11px', fontWeight:'bold' }}>Published</span>
                                <span style={{ color:'#aaa' }}>v{selected.house_rules_version}</span>
                                {selected.house_rules_effective_date && (
                                  <span style={{ color:'#888' }}>· effective {selected.house_rules_effective_date}</span>
                                )}
                                <button
                                  onClick={() => caOpenHouseRulesModal(selected)}
                                  style={{ background:'none', border:'none', color:'#C9A227', fontSize:'12px', textDecoration:'underline', cursor:'pointer', padding:0, fontFamily:'inherit' }}
                                >View rules</button>
                              </div>
                            ) : selected.house_rules_version > 0 ? (
                              <div style={{ fontSize:'12.5px', color:'#aaa', display:'flex', alignItems:'center', gap:'10px', flexWrap:'wrap' }}>
                                <span style={{ background:'#1a1400', color:'#fbbf24', padding:'2px 8px', borderRadius:'10px', fontSize:'11px', fontWeight:'bold' }}>Unpublished</span>
                                <span style={{ color:'#555', fontSize:'11px' }}>· previously v{selected.house_rules_version}, view history for prior text</span>
                                <button
                                  onClick={() => caOpenHouseRulesModal(selected)}
                                  style={{ background:'none', border:'none', color:'#C9A227', fontSize:'12px', textDecoration:'underline', cursor:'pointer', padding:0, fontFamily:'inherit' }}
                                >View history</button>
                              ...
                              <div style={{ fontSize:'12.5px', color:'#aaa' }}>
                                <span style={{ background:'#1e2535', color:'#888', padding:'2px 8px', borderRadius:'10px', fontSize:'11px', fontWeight:'bold' }}>Not published</span>
                                <span style={{ marginLeft:'10px', color:'#555', fontSize:'11px' }}>· property manager authors in the manager portal</span>
                              </div>
                            )}
                          </div>
            ...
            {manageSection === 'users' && CA_CRM_REDESIGN && (() => {
                ...
                return (
                  <div style={{ background:'#161b26', border:'1px solid #2a2f3d', borderRadius:'10px', padding:'12px 14px', marginBottom:'8px' }}>
                    ...
                    <div style={{ display:'flex', gap:'6px', marginTop:'8px', flexWrap:'wrap' }}>
                      ...
                      {/* 2026-08-08 §1 — Deactivate/Activate for drivers.
                          Handler mirrors toggleUserActive's swift-handler
                          shape (auth ban is load-bearing; column writes
                          best-effort; explicit severity in result banner).
                          `active` follows u.is_active which is base.is_active,
                          i.e. user_roles.is_active when the roleRow lookup
                          succeeds — the state get_my_effective_active
                          reads at portal mount. u.__drivers_id is the
                          drivers-table primary key needed by step 3's
                          drivers.update(...).eq('id', ...) target. */}
                      {isDriver && (
                        active
                          ? <button onClick={() => toggleDriverActive({ id: u.__drivers_id, email: u.email, is_active: true })} disabled={togglingUser === u.email}
                              style={{ padding:'4px 10px', background:'#3a1a1a', color:'#f44336', border:'1px solid #b71c1c', borderRadius:'5px', cursor:'pointer', fontSize:'11px' }}>Deactivate</button>
                          : <button onClick={() => toggleDriverActive({ id: u.__drivers_id, email: u.email, is_active: false })} disabled={togglingUser === u.email}
                              style={{ padding:'4px 10px', background:'#1a3a1a', color:'#4caf50', border:'1px solid #2e7d32', borderRadius:'5px', cursor:'pointer', fontSize:'11px' }}>Activate</button>
                      )}
              ...
              function DriverRow({ d }: { d: any }) {
                ...
                const base = roleRow || { email: d.email, role: 'driver', name: d.name,
                ...
                // 2026-08-08 §1 — carry drivers-table id onto the composite
                // so PeopleRow's Deactivate button can pass the drivers
                // row shape to toggleDriverActive. base.id (when roleRow
                // wins) is user_roles.id — NOT the same, and would miss
                // the drivers.update(...).eq('id', ...) target. Distinct
                // key __drivers_id preserves that separation. Divergence
                // between user_roles.is_active (base.is_active for
                // roleRow) and drivers.is_active is captured too — the
                // active label in PeopleRow follows base.is_active, and
                // toggleDriverActive writes both tables to !wasActive on
                // click so any historical drift heals to a consistent
                // state on the next toggle.
                const u = { ...base, __drivers_id: d.id }
